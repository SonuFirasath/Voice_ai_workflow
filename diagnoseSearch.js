...
async function run() {
    ...
    if (!tokenData.access_token) {
        console.error("   FAILED:", tokenData.error_description || JSON.stringify(tokenData));
        return;
    }
    ...
    if (usersData.value) {
        ...
        for (const u of usersData.value) {
            ...
            console.log(`   • ${u.displayName} | ID: ${u.id} | Phones: ${phones.length > 0 ? phones.join(', ') : '(none)'} | Title: ${u.jobTitle || '(none)'}`);
        }
    } else {
    ...
    const schemaRes = await fetch(`${endpoint}/indexes/sharepoint-index?api-version=${API_VERSION}`, {
    ...
    if (schema.error) {
        ...
        for (const f of schema.fields) {
            console.log(`   • ${f.name} (${f.type}) | filterable: ${f.filterable || false} | permissionFilter: ${f.permissionFilter || 'none'}`);
        }
    }
    ...
    const client = new SearchClient(endpoint, "sharepoint-index", new AzureKeyCredential(SEARCH_KEY));
    ...
    try {
        ...
        if (totalDocs === 0) {
            ...
            console.log("   → Indexer may not have completed, or SharePoint auth failed.");
            ...
            if (emptyDocs === totalDocs) {
                ...
                console.log("   → PDFs are almost certainly scanned images with no text layer.");
                console.log("   → Fix: Add OCR cognitive skill to the indexer (tell Claude to do this).");
            } else {
    ...
    try {
        ...
        for await (const r of userIdResults.results) {
            ...
            console.log(`   • "${doc.title}" → UserIds: [${userIds.join(', ')}] | GroupIds: [${groupIds.join(', ')}]`);
        }
        if (docCount === 0) {
            console.log("   No documents returned.");
        }
    } catch (e) {
        console.log(`   ⚠️  Could not retrieve UserIds (may be non-retrievable): ${e.message}`);
        ...
        try {
            ...
            if (restData.value) {
                for (const doc of restData.value) {
                    console.log(`   • "${doc.title}" → UserIds: ${JSON.stringify(doc.UserIds)} | GroupIds: ${JSON.stringify(doc.GroupIds)}`);
                }
            } else {
    ...
    if (usersData.value && usersData.value.length > 0) {
        ...
        try {
            ...
            if (fCount === 0) {
                ...
                console.log("   → This means the UserIds field does NOT contain this user's Entra Object ID.");
                console.log("   → The SharePoint indexer may not have populated ACLs, or the user doesn't have permission to the SharePoint files.");
            } else {
                console.log(`   ✅ Found ${fCount} documents accessible to ${testUser.displayName}.`);
            }
        } catch (e) {
