...
async function run() {
  ...
  // Delete if exists
  await fetch(`${BASE_URL}/indexers/test-indexer?api-version=${API_VERSION}`, {
    method: "DELETE",
    headers,
  });
  await fetch(`${BASE_URL}/indexes/test-index?api-version=${API_VERSION}`, {
    method: "DELETE",
    headers,
  });
  ...
  const idxRes = await fetch(
    `${BASE_URL}/indexes/test-index?api-version=${API_VERSION}`,
    {
      method: "PUT",
      headers,
      body: JSON.stringify(testIndexPayload),
    },
  );
  if (!idxRes.ok) {
  ...
  const indxrRes = await fetch(
    `${BASE_URL}/indexers/test-indexer?api-version=${API_VERSION}`,
    {
      method: "PUT",
      headers,
      body: JSON.stringify(testIndexerPayload),
    },
  );
  if (!indxrRes.ok) {
  ...
  const statusRes = await fetch(
    `${BASE_URL}/indexers/test-indexer/status?api-version=${API_VERSION}`,
    { headers },
  );
  const status = await statusRes.json();
  if (status.lastResult) {
    console.log(
      `   Status: ${status.lastResult.status} | Processed: ${status.lastResult.itemsProcessed} | Failed: ${status.lastResult.itemsFailed}`,
    );
  }
  ...
  const searchRes = await fetch(
    `${BASE_URL}/indexes/test-index/docs/search?api-version=${API_VERSION}`,
    {
      method: "POST",
      headers,
      body: JSON.stringify({
  ...
  if (searchData.value && searchData.value.length > 0) {
    ...
    console.log(
      "   ❌ Still 0 documents. The SharePoint indexer itself may not be pulling content.",
    );
    if (searchData.error)
  ...
  if (searchData.value && searchData.value.length > 0) {
    ...
    if (allUserIds.size > 0) {
      console.log(
        `   Found ${allUserIds.size} unique UserIds in documents. Looking up names...\n`,
      );
      for (const uid of allUserIds) {
        try {
          const userRes = await fetch(
            `https://graph.microsoft.com/v1.0/users/${uid}?$select=displayName,mobilePhone,userPrincipalName`,
            {
          ...
          if (userData.displayName) {
            console.log(
              `   • ${uid} → ${userData.displayName} (${userData.userPrincipalName}) | Phone: ${userData.mobilePhone || "(none)"}`,
            );
          } else {
            console.log(
              `   • ${uid} → Not found in Entra (may be a group or external user)`,
            );
          }
        } catch (e) {
      ...
      console.log(
        "   ⚠️ Documents have EMPTY UserIds — the SharePoint ACLs may use Groups only.",
      );
    }
  }
