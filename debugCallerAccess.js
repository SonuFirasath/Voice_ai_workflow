...
async function run() {
    ...
    const conditions = variants.map(v => `mobilePhone eq '${v}' or businessPhones/any(p:p eq '${v}')`).join(' or ');
    const userRes = await graphGet(token,
    ...
    const groupRes = await graphGet(token,
    ...
    for (const g of groups) {
        ...
        const results = await client.search("*", { filter, select: ["title"], top: 10 });
