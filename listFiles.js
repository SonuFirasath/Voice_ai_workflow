...
async function listAccessibleFiles() {
    ...
    if (hits.length === 0) {
        console.log("0 files found. (The crawler is still indexing, or the folder is empty).");
    } else {
        hits.forEach((hit, index) => {
            const file = hit.resource;
            console.log(`[${index + 1}] File Name: ${file.name}`);
            console.log(`    Last Modified: ${new Date(file.lastModifiedDateTime).toLocaleString()}`);
