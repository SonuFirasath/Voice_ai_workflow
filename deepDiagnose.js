...
async function run() {
  ...
  console.log(
    `   Document count (from schema): Check Azure Portal for exact count.`,
  );
  ...
  if (postData.value) {
    for (const doc of postData.value) {
      console.log(
        `   • "${doc.title}" → ${doc.metadata_spo_item_path || "(no path)"}`,
      );
    }
  }
  if (postData.error) {
    ...
    if (siteData.id) {
      ...
      if (permsData.value) {
        ...
        for (const perm of permsData.value) {
          const identity =
            perm.grantedToIdentitiesV2 || perm.grantedToIdentities || [];
          console.log(
            `   • ${JSON.stringify(perm.roles)} → ${JSON.stringify(identity).substring(0, 200)}`,
          );
        }
      } else {
        console.log(
          `   Permissions response: ${JSON.stringify(permsData).substring(0, 300)}`,
        );
      }
    } else {
      console.log(
        `   Site lookup: ${JSON.stringify(siteData).substring(0, 300)}`,
      );
    }
  } catch (e) {
  ...
  console.log(
    `   Results with UserIds filter: ${filteredData.value?.length || 0}`,
  );
  ...
  console.log(
    `   Results with 'Everyone' group filter: ${groupData.value?.length || 0}`,
  );
  if (groupData.value) {
  ...
  if (histData.executionHistory) {
    for (const exec of histData.executionHistory.slice(0, 3)) {
      console.log(
        `   Run: ${exec.status} | Processed: ${exec.itemsProcessed} | Failed: ${exec.itemsFailed} | Start: ${exec.startTime}`,
      );
