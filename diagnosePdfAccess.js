...
async function run() {
    ...
    if (!tokenData.access_token) {
        console.error("❌ AUTHENTICATION FAILED:", JSON.stringify(tokenData, null, 2));
        return;
    }
    ...
    if (!siteResponse.ok) {
        ...
        console.error("   -> Check that Sites.Read.All permission has Admin Consent.");
        return;
    }
    ...
    console.log("=" .repeat(60));
    console.log("This checks if the service principal can directly browse each folder.\n");

    const folderNames = ["General_Policies", "Manager_Policies", "Sales_Policies"];
    ...
    for (const folderName of folderNames) {
        ...
        if (!listResponse.ok) {
            console.error(`   ❌ CANNOT LIST: ${listResponse.status} ${listResponse.statusText}`);
    ...
    for (const [name, folderUrl] of Object.entries(folders)) {
        ...
        const listRes = await fetch('https://graph.microsoft.com/v1.0/search/query', {
            method: 'POST',
        ...
        const moreAvailable = listData.value?.[0]?.hitsContainers?.[0]?.moreResultsAvailable || false;

        if (hits.length === 0) {
            console.log(`   ⚠️  0 indexed documents found.`);
            console.log(`   Possible causes:`);
            console.log(`     - Files were uploaded recently and haven't been indexed yet (wait 5-15 min).`);
            console.log(`     - The folder URL doesn't match what SharePoint uses internally.`);
            ...
            const broadRes = await fetch('https://graph.microsoft.com/v1.0/search/query', {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
            ...
            console.log(`   Fallback (without wildcard): ${broadHits.length} result(s)`);
        } else {
            console.log(`   ✅ ${hits.length} indexed document(s) found (total: ${totalRows}, more: ${moreAvailable})`);
            hits.forEach(hit => {
    ...
    console.log("=" .repeat(60));
    console.log("Searching for a generic term to test full-text search inside documents.\n");
    ...
    for (const term of testTerms) {
        ...
        if (searchHits.length > 0) {
            console.log(`   ✅ "${term}" → ${searchHits.length} hit(s) in General_Policies`);
    ...
    console.log("=" .repeat(60));
    console.log("Attempting to download the first PDF found in General_Policies.\n");
    ...
    if (pdfListRes.ok) {
        ...
        const firstPdf = (pdfListData.value || []).find(f => f.name?.toLowerCase().endsWith('.pdf'));

        if (firstPdf) {
            console.log(`   Found PDF: ${firstPdf.name} (${(firstPdf.size / 1024).toFixed(1)} KB)`);
            ...
            if (downloadRes.ok) {
                ...
                console.log(`   First 4 bytes (should be %PDF): ${buffer.toString('ascii', 0, 4)}`);
                ...
                try {
                    ...
                    console.log(`   (This means the file downloaded fine, but pdf-parse couldn't read it.)`);
                }
            } else {
