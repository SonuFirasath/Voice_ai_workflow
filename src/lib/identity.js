...
async function lookupCaller(accessToken, callerNumber) {
    ...
    console.log(`[IDENTITY] Trying ${variants.length} phone variants: ${JSON.stringify(variants)}`);
    ...
    console.log(`[IDENTITY] Found user: ${data.value[0].displayName} (ID: ${data.value[0].id})`);
