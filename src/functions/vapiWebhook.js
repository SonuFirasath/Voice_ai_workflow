  ...
  handler: async (request, context) => {
    ...
    try {
      ...
      if (!accessToken) {
        ...
        return {
          status: 500,
          jsonBody: { error: "Authentication with Entra ID failed." },
        };
      }
      ...
      if (callerNumber) {
        ...
        if (!caller) {
          context.log(
            `[WEBHOOK] CALLER NOT FOUND in Entra ID: ${callerNumber}`,
          );
          ...
          if (eventType === "assistant-request") {
            context.log(
              "[WEBHOOK] Returning assistant config despite unidentified caller.",
            );
            const host =
              request.headers.get("x-forwarded-host") ||
              request.headers.get("host");
            const publicUrl = host
              ? `https://${host}/api/vapiWebhook`
              : undefined;
            return {
              status: 200,
              jsonBody: buildAssistantConfig(employeeName, publicUrl),
            };
          }
              ...
              results: [
                {
                  toolCallId:
                    ...
                    "unknown",
                  result:
                    "I'm sorry, but I couldn't verify your identity in our directory. Please contact IT support to ensure your phone number is registered.",
                },
              ],
            },
          };
        ...
        context.log(
          "[WEBHOOK] WARNING: No caller number in payload — skipping identity lookup.",
        );
      }
      ...
      if (eventType === "assistant-request") {
        context.log(
          `[WEBHOOK] Returning assistant config for: ${employeeName}`,
        );
        ...
        context.log(
          `[WEBHOOK] Server URL for tool callbacks: ${publicUrl || "NOT RESOLVED"}`,
        );
      ...
      if (eventType === "tool-calls") {
        ...
        if (!toolCall) {
          context.log(
            `[WEBHOOK] ERROR: No tool call found. Message keys: ${Object.keys(body.message)}`,
          );
          return {
            status: 200,
            jsonBody: {
              results: [
                {
                  toolCallId: "unknown",
                  result:
                    "I encountered an internal error. Please try asking your question again.",
                },
              ],
            },
          };
        ...
        context.log(
          `[WEBHOOK] Tool called: ${toolCall.function?.name} | toolCallId: ${toolCallId}`,
        );
        ...
        if (toolCall.function?.name === "search_sharepoint") {
          ...
          try {
            ...
            context.log(
              `[WEBHOOK] ERROR: Failed to parse tool arguments: ${parseErr.message}`,
            );
            return {
              status: 200,
              jsonBody: {
                results: [
                  {
                    toolCallId,
                    result:
                      "I had trouble understanding the request. Could you rephrase your question?",
                  },
                ],
              },
            };
          ...
          context.log(
            `[WEBHOOK] Search query: "${args.query}" | Caller ID: "${callerId}"`,
          );

          if (!callerId) {
            context.log(
              "[WEBHOOK] CRITICAL: callerId is null at search time — identity lookup must have failed.",
            );
          }
                ...
                results: [
                  {
                    toolCallId,
                    result:
                      "I'm having trouble accessing the document system right now. Please try again in a moment.",
                  },
                ],
              },
            };
          ...
          context.log(
            `[WEBHOOK] Search success: returning ${result.text.length} chars to AI.`,
          );
          return {
            status: 200,
            jsonBody: {
