...
const client   = new SearchClient(endpoint, "sharepoint-index", new AzureKeyCredential(process.env.SEARCH_API_KEY));
...
async function check() {
    ...
    if (total === 0) {
        ...
        console.log("   Even though Azure Portal shows 6 docs, the API returns nothing.");
        console.log("   This is likely caused by permissionFilterOption blocking unfiltered queries.");
        console.log("   → Re-run setupSearch.js after removing permissionFilterOption from the index.");
    } else if (emptyCount === total) {
        console.log("\n❌ ALL DOCUMENTS HAVE EMPTY CONTENT.");
        console.log("   Your PDFs are scanned images with no selectable text layer.");
        ...
        const testResults = await client.search("policy", { select: ["title"], top: 5 });
        let found = 0;
        ...
        if (found === 0) {
            console.log("   ⚠️  Keyword 'policy' matched nothing despite content existing.");
            console.log("   → The content field may contain non-searchable/corrupted text.");
        } else {
            console.log(`\n✅ Keyword search works — ${found} result(s) for 'policy'.`);
            console.log("   The index and search are working correctly.");
            console.log("   → The problem is in the VAPI/search.js call path, not the index.");
        }
    }
}
