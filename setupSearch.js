...
async function provisionAzureSearch() {
  try {
    ...
    const dsRes = await fetch(
      `${BASE_URL}/datasources/sharepoint-datasource?api-version=${API_VERSION}`,
      {
        method: "PUT",
        headers,
    ...
    if (!indxrRes.ok)
      throw new Error(`Indexer Error: ${await indxrRes.text()}`);
    ...
    if (forceReset) {
      console.log(
        "4a. Resetting indexer state (--reset flag detected) — forces full re-crawl...",
      );
      const resetRes = await fetch(
        `${BASE_URL}/indexers/sharepoint-indexer/reset?api-version=${API_VERSION}`,
        {
          method: "POST",
          headers,
        },
      ...
      if (!resetRes.ok && resetRes.status !== 204) {
        console.warn(
          `   WARNING: Reset returned ${resetRes.status}: ${await resetRes.text()}`,
        );
      } else {
        console.log(
          "   -> Indexer state reset. All documents will be re-crawled and UserIds/GroupIds repopulated.",
        );
      }
    }
    ...
    const runRes = await fetch(
      `${BASE_URL}/indexers/sharepoint-indexer/run?api-version=${API_VERSION}`,
      {
        method: "POST",
        headers,
      },
    ...
    if (runRes.status !== 202 && runRes.status !== 409) {
      throw new Error(
        `Run trigger failed (${runRes.status}): ${await runRes.text()}`,
      );
    }
    ...
    for (let attempt = 1; attempt <= 36; attempt++) {
      ...
      if (running) {
        process.stdout.write(
          `\r   -> Still running... (${attempt * 5}s elapsed)`,
        );
        continue;
      }
      ...
      if (last.errors && last.errors.length > 0) {
        ...
        last.errors
          .slice(0, 5)
          .forEach((e) => console.log(`     • ${e.errorMessage || e.message}`));
      }
      done = true;
      break;
    }
