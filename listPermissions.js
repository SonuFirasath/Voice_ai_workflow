...
async function listFilesRecursive(token, driveId, itemId, folderPath, results) {
    ...
    while (nextUrl) {
        ...
        for (const item of (data.value || [])) {
            if (item.folder) {
                await listFilesRecursive(token, driveId, item.id, `${folderPath}/${item.name}`, results);
            } else if (item.file) {
...
function formatPerm(perm) {
    ...
    if (perm.grantedToIdentitiesV2?.length) {
        return perm.grantedToIdentitiesV2.map(identity => {
            if (identity.user)  return `   👤 ${identity.user.displayName || identity.user.email} — ${roles}`;
            if (identity.group) return `   👥 GROUP: ${identity.group.displayName} — ${roles}`;
...
async function run() {
    ...
    const site = await graphGet(token, `https://graph.microsoft.com/v1.0/sites/${hostname}:${sitePath}`);
    console.log(`✅ Site: "${site.displayName}" (ID: ${site.id})\n`);

    // List drives (document libraries)
    const drivesData = await graphGet(token, `https://graph.microsoft.com/v1.0/sites/${site.id}/drives`);
    ...
    if (allFiles.length === 0) {
        console.log("No files found. Check that SP_SITE_URL points to the correct site and the app has Sites.Read.All permission.");
        return;
    }
    ...
    for (const file of allFiles) {
        ...
        try {
            const permsData = await graphGet(token,
                `https://graph.microsoft.com/v1.0/drives/${defaultDrive.id}/items/${file.id}/permissions`
            );

            const perms = permsData.value || [];
            if (perms.length === 0) {
                console.log("   (no explicit permissions — fully inherits from parent)");
                continue;
            }

            for (const perm of perms) {
                const inherited = perm.inheritedFrom ? ` [inherited from ${perm.inheritedFrom.path}]` : '';
                const line = formatPerm(perm);
                console.log(line + inherited);
            }
        } catch (e) {
            console.log(`   ⚠️  Could not fetch permissions: ${e.message.split('\n')[0]}`);
        }
    }
...
run().catch(err => {
    ...
    if (err.message.includes('403') || err.message.includes('Forbidden')) {
        ...
        console.error('  Go to: Azure Portal → App Registrations → your app → API Permissions → Add permission → Microsoft Graph → Application → Sites.Read.All → Grant admin consent');
    }
});
