...
async function deleteIfExists(resourceType, name) {
    ...
    if (res.ok || res.status === 404) {
        ...
        console.error(`   ❌ Failed to delete ${resourceType}/${name}: ${res.status} ${text}`);
    }
}
...
async function rebuild() {
    ...
    const statusRes = await fetch(`${BASE_URL}/indexers/sharepoint-indexer/status?api-version=${API_VERSION}`, {
        headers
    });
    ...
    const client = new SearchClient(`${BASE_URL}`, "sharepoint-index", new AzureKeyCredential(SEARCH_KEY));
    ...
    try {
        ...
        if (count === 0) {
            console.log("   ⚠️ Still empty — indexer may need more time. Try running diagnoseSearch.js in a few minutes.");
        } else {
    ...
    console.log("\n" + "=".repeat(60));
    console.log("  Done. If index is still empty, wait 2-3 minutes and run diagnoseSearch.js");
    console.log("=".repeat(60));
}
