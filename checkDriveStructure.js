...
async function run() {
    ...
    for (const path of pathAttempts) {
        ...
        if (res.ok) {
            ...
            data.value.forEach(item => {
                console.log(`      ${item.folder ? "📁" : "📄"} ${item.name} (${(item.size/1024).toFixed(1)} KB)`);
            });
        } else {
    ...
    for (const region of regions) {
        ...
        if (searchData.error) {
            console.log(`Region "${region}": ❌ ${searchData.error.message.substring(0, 100)}`);
        } else {
            const hits = searchData.value?.[0]?.hitsContainers?.[0]?.hits || [];
            const total = searchData.value?.[0]?.hitsContainers?.[0]?.total || 0;
            console.log(`Region "${region}": ✅ ${hits.length} hits (total: ${total})`);
            if (hits.length > 0) {
