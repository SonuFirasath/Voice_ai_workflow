...
async function test() {
    ...
    for (const [groupName, groupId] of Object.entries(GROUPS)) {
        ...
        try {
            ...
            if (count === 0) {
                console.log(`  ⚠️  ZERO results — GroupIds not populated for this group`);
            } else {
                anyGroupWorked = true;
            }
        } catch (e) {
    ...
    console.log("\n" + "=".repeat(60));
    if (anyGroupWorked) {
        console.log("✅ GroupIds ARE populated — ready to implement group-based search filter.");
    } else {
        console.log("❌ GroupIds are NOT populated — the indexer did not extract Entra group ACLs.");
        console.log("\nPossible causes:");
        console.log("  1. The field mapping source name 'metadata_group_ids' may be incorrect");
        console.log("  2. M365 group permissions on SharePoint may not be resolved by the indexer");
        console.log("  3. The app registration may be missing Sites.FullControl.All permission");
    }
    console.log("=".repeat(60));
}
